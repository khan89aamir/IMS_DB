...
-- =============================================
-- EXEC [dbo].[Insert_PurchaseInvoice_BulkPrint_Color_Size] 2,1,100,1,'purinv02',0
...
BEGIN
	...
	SET @query2='
	SELECT ColorID,Color,ProductID,ProductName,QTY,Size,ModelNo,Rate,StoreID FROM
	(SELECT clr.ColorName AS Color,
	clr.ColorID,pd1.ProductID,pm.ProductName,pd1.ModelNo,pm.Rate,pd1.StoreID,'+@query1+'pd3.Total,FROM DeliveryPurchaseBill1 pd1
	...
	WHERE 
	pd1.PurchaseInvoiceID='+CAST(@PurchaseInvoiceID AS VARCHAR)+' AND pd2.DeliveryPurchaseID1='+CAST(@DeliveryPurchaseID as VARCHAR)+' group by	pd1.ProductID,pm.ProductName,clr.ColorID,clr.ColorName,pd1.ModelNo,pm.Rate,pd1.StoreID,pd3.Total)a UNPIVOT
	(
	QTY
